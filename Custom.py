...
def get_search_chain(name, set, personality, line, situation): # 인격을 지정하기 위해 필요한 데이터를 가져오는 코드
    def get_data(input_variables):
        ...
        return {"name": name, "set": set, "line": line, "situation": situation, "personality": personality}
    
    search_chain = TransformChain(input_variables=["chat"], output_variables=["name", "set", "line", "situation", "personality"], transform=get_data)
    return search_chain
...
def get_chatgpt_chain(): # GPT-4를 사용하여 대화를 생성하는 코드
    ...
    위에서 설명한 'bot'의 설정, 성격, 대사, 상황을 참고해서 'bot'의 말투로 'you'와 상황에 맞춰서 대화해 줘.
...
class Custom2:
    def __init__(self, name, set, personality, line, situation) -> None:
        self.memory = get_memory()
        self.search_chain = get_search_chain(name, set, personality, line, situation)
